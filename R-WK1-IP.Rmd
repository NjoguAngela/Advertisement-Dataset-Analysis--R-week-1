...
#clicked_ad <- as.data.frame(filter(Topics, Clicked.on.Ad == 1))
...
did_click_h <- filter(Hours, Clicked.on.Ad == 1)
didnt_click_h <- filter(Hours, Clicked.on.Ad == 0)
...
The variable with the highest coefficient is Daily internet usage closely followed by Daily time spent on site. The gender coefficient is very low meaning the variable cannot be reliably used to determine whether an individual would click on an add or not. The area income and age variable coefficients indicate that the variables can effect a change in the clicked.on.ad variable but not that much. The variables cannot be used individual to determine whether an individual would click on an ad or not.
